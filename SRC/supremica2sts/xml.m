function [ Event,State,Transitiontable,Statetable] = xml(filename)
    ...
    for i=1:Num_Events
        ...
        Strtemp = regexp(Event{i,2}, ':', 'split');
        Event{i,2} = Strtemp{1};
    ...
    for i=1:Num_States
        ...
        State1accepting=char(State1.getAttribute('accepting'));
        State{i,1}=State1id;
    ...
    for i=1:m2
        if strcmp(State(i,3),'true')==1
            Statetable(j)=State(i,1);
            j=j+1;
        end
    end
    [m1,n1]=size(Statetable);
    for i=1:m1
    for j=1:n1
    Statetable{i,j}=str2double(Statetable{i,j});
    end
    end
    Statetable=cell2mat(Statetable);
end
